 ...
 *                                 or file path
 * @param {string} name - the 'name' of the source if loading from a variable
 *
 * @throws {TypeError} if file cannot be opened
 *
 * @return {object} JSON document
 */
export function loadJSON(source, name = 'source') {
 ...
 *                                     configurationo
 *
 * @return {object} PDF object that can be further piped and processed
 */
export function templateToPDF(
  ...
  const contextParsed = context ? loadJSON(context, 'context') : {};
  const settingsParsed = settings ? loadJSON(settings, 'settings') : {};
  ...
  if (destination) {
    pdfResult.toFile(destination, (err) => {
      if (err) {
        throw err;
      } else {
        console.debug(`PDF written to: ${destination}`);
      }
    });
  }
